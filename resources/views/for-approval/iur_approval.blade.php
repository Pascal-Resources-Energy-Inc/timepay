  ...
  <div class="content-wrapper">
          ...
          <div class="card-body">
            <div class="media">                
              <div class="media-body">
                ...
                <a href="/coe-approval?status=Pending" class="h2 card-text text-white">{{$for_approval}}</a>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card-body">
            <div class="media">                
              <div class="media-body">
                ...
                <a href="/coe-approval?status=Approved" class="h2 card-text text-white">{{$approved}}</a>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card-body">
            <div class="media">                
              <div class="media-body">
                ...
                <a href="/coe-approval?status=Declined" class="h2 card-text text-white">{{$declined}}</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class='row'>
      <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">ID and Uniform Approval Requests</h4>
            ...
            <div class="table-responsive">
              <table class="table table-hover table-bordered tablewithSearch">
                ...
                <tbody> 
                    @foreach ($iurs as $form_approval)
                      <tr>
                        {{-- Checkbox for bulk actions (only for pending items that current user can approve) --}}
                        @if((empty($status) || $status == 'Pending') && $form_approval->status == 'Pending')
                          <td align="center">
                        ...
                        <td align="center" id="tdActionId{{ $form_approval->id }}" data-id="{{ $form_approval->id }}">
                          ...
                          @elseif($form_approval->status == 'Pending')
                            ...
                            <button type="button" class="btn btn-success btn-sm" data-target="#iur-approved-remarks-{{ $form_approval->id }}" data-toggle="modal" title="Approve">
                              <i class="ti-check btn-icon-prepend"></i>                                                    
                            </button>
                            <button type="button" class="btn btn-danger btn-sm" data-target="#iur-declined-remarks-{{ $form_approval->id }}" data-toggle="modal" title="Decline">
                        ...
                        <td>{{ $form_approval->iur_reference }}</td>
                        <td>{{ date('M. d, Y', strtotime($form_approval->created_at)) }}</td>
                        <td>{{ $form_approval->user->employee_code }}</td>
                        <td>
                          <strong>{{ $form_approval->user->first_name . ' ' . $form_approval->user->last_name }}</strong> <br>
                          <small>Position: {{$form_approval->user->position ?? 'N/A'}}</small> <br>
                          <small>Location: {{$form_approval->user->location ?? 'N/A'}}</small> <br>
                          <small>Department: {{ $form_approval->user->department ? $form_approval->user->department->name : "N/A"}}</small>
                        </td>
                        <td>{{ $form_approval->user->original_date_hired }}</td>
                        <td>{{$form_approval->purpose ?? $form_approval->type ?? '-'}}</td>
...
$(document).ready(function() {
    ...
    function sendAjax(url, successMessage) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to ${successMessage.toLowerCase()} ${selectedItems.length} IUR(s).`,
            icon: 'question',
    ...
    $approveBtn.on('click', function() {
        sendAjax('/approve-iur-all', 'Approved');
    });

    $disapproveBtn.on('click', function() {
        sendAjax('/disapprove-iur-all', 'Disapproved');
    });
...
@foreach ($iurs as $form_approval)
  @include('for-approval.remarks.iur_approved_remarks', ['form_approval' => $form_approval])
  @include('for-approval.remarks.iur_declined_remarks', ['form_approval' => $form_approval])
  @include('for-approval.view-iur', ['form_approval' => $form_approval])
@endforeach
