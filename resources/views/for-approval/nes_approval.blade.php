    ...
    <div class="content-wrapper">
        ...
        <div class='row'>
          <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">For Approval Number Enrollment</h4>
                  ...
                  <table class="table table-hover table-bordered tablewithSearch">
                    ...
                    <tbody> 
                      @foreach ($nes as $form_approval)
                         ...
                         @if((empty($status) || $status == 'Pending'))
                          ...
                          <td align="center" id="tdActionId{{ $form_approval->id }}" data-id="{{ $form_approval->id }}">
                            @if($form_approval->status == 'Approved')
                              <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#view-ne-{{ $form_approval->id }}" title="View Approved">
                                <i class="ti-eye btn-icon-prepend"></i> View
                              </button>
                            @elseif($form_approval->status == 'Declined')
                              <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#view-ne-{{ $form_approval->id }}" title="View Declined Remarks">
                                <i class="ti-eye btn-icon-prepend"></i> View
                              </button>
                            @else
                              @if($form_approval->status == 'Pending')
                                <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#view-ne-{{ $form_approval->id }}" title="View">
                                  <i class="ti-eye btn-icon-prepend"></i> 
                                </button>
                                <button type="button" class="btn btn-success btn-sm" id="{{ $form_approval->id }}" data-target="#ne-approved-remarks-{{ $form_approval->id }}" data-toggle="modal" title="Approve">
                                  <i class="ti-check btn-icon-prepend"></i>                                                    
                                </button>
                                <button type="button" class="btn btn-danger btn-sm" id="{{ $form_approval->id }}" data-target="#ne-declined-remarks-{{$form_approval->id}}" data-toggle="modal" title="Decline">
                        ...
                        <td>{{ $form_approval->cellphone_number ?? 'N/A' }}</td>
                        <td>{{ ucfirst(str_replace('_', ' ', $form_approval->network_provider ?? 'N/A')) }}</td>
                        <td>{{ ucfirst(str_replace('_', ' ', $form_approval->enrollment_type ?? 'N/A')) }}</td>
                        <td>
                          @if ($form_approval->status == 'Pending')
...
$(document).ready(function() {
    ...
    function sendAjax(url, successMessage) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: `You are about to ${successMessage.toLowerCase()} ${selectedItems.length} NE(s).`,
            icon: 'question',
    ...
    $approveBtn.on('click', function() {
        sendAjax('/approve-ne-all', 'Approved');
    });

    $disapproveBtn.on('click', function() {
        sendAjax('/disapprove-ne-all', 'Disapproved');
    });
...
@foreach ($nes as $ne)
   ...
   @include('for-approval.remarks.ne_approved_remarks')
   @include('for-approval.remarks.ne_declined_remarks')
@endforeach
