@foreach($tos as $form_approval)
  <div class="modal fade" id="to-view-modal-{{ $form_approval->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                  ...
                  <div class="p-1 text-center">
                   @if ($form_approval->show_final_approver)
                              <input type="text"
                                  style="background: transparent; text-align: center; border: none; border-bottom: 1px solid #000; box-shadow: none; height: 30px; padding-left: 0;"
                                  name="approved_by"
                                  value="{{ $form_approval->approvedByHeadDivision->name ?? $approver->approver_info->name }}"
                                  readonly>
                      @else
                          <input type="text"
                              style="background: transparent; text-align: center; border: none; border-bottom: 1px solid #000; box-shadow: none; height: 30px; padding-left: 0;"
                              name="approved_by"
                              value="{{ $form_approval->approvedByHeadDivision->name ?? $form_approval->approver->first()->approver_info->name ?? ($form_approval->employee->immediateSupervisor->first_name . ($form_approval->employee->immediateSupervisor->middle_initial ? ' ' . $form_approval->employee->immediateSupervisor->middle_initial . '.' : '') . ' ' . $form_approval->employee->immediateSupervisor->last_name) }}"
                              readonly>
                      @endif
...
document.addEventListener('DOMContentLoaded', function() {
    @foreach($tos as $form_approval)
        (function() {
            const modal = document.querySelector('#to-view-modal-{{ $form_approval->id }}');
